...
5. Sign in with username `demo`. Find the generated `DEMO_PASSWORD` in the service's Environment settings; keep it private. It is generated by Render, not committed in the repository.

If creating a Web Service manually instead of using a Blueprint, choose Docker with the repository root as the build context, the root Dockerfile, and Free compute. Set `SPRING_PROFILES_ACTIVE=render`, `DEMO_USERNAME=demo`, and a private `DEMO_PASSWORD` of at least 16 characters. The remaining demo providers default to mock/Wikipedia. The Render profile reads `PORT` automatically and binds to `0.0.0.0`.

The Render profile enables session login and CSRF protection. All app pages and API endpoints require authentication except the health check and login flow. This is still a shared demo account, not enterprise multi-tenant authorization. Do not share its password publicly. Session cookies are HTTPS-only. Local development keeps the existing local demo behavior.
